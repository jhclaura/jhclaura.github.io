...
THREE.DeviceControls = function ( camera, worldCenter ) {
	...
	function myMouseDown(event) {
		...
		topOpen = !topOpen;
		if(topOpen){
			new TWEEN.Tween(pocketTop.rotation)
							.to({x: 0}, 2000)
							.easing(TWEEN.Easing.Elastic.InOut)
							.start();
			
			setTimeout(function(){
				new TWEEN.Tween(pocketTop.children[5].rotation)
								.to({x: 0}, 2000)
								.easing(TWEEN.Easing.Elastic.InOut)
								.start();
				new TWEEN.Tween(pocketTop.children[5].children[0].rotation)
								.to({x: 0}, 3000)
								.easing(TWEEN.Easing.Elastic.InOut)
								.start();
				new TWEEN.Tween(pocketTop.children[5].children[1].scale)
								.to({y: 1}, 3000)
