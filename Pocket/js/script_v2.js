...
{	
	...
	// PLAYERS
		...
		loader.load( "models/Guy2/GuyB.js", function( geometry ){
			...
			var fGuy = new THREE.Mesh( geometry.clone(), new THREE.MeshLambertMaterial( { map: skinTexture, color: 0xff0000, side: THREE.DoubleSide } ) );				
			fGuy.name = "body";
			fGuy.position.z = -0.1;	//
			firstGuyBody.add(fGuy);

			var sGuy = new THREE.Mesh( geometry.clone(), new THREE.MeshLambertMaterial( { map: skinTexture, color: 0x00ff00, side: THREE.DoubleSide } ) );
			sGuy.name = "body";
			sGuy.position.z = -0.1;	//
			...
			loader.load( "models/Guy2/GuyLA.js", function( geometry2 ){
				...
				fGuy = new THREE.Mesh( tmpLA.clone(), new THREE.MeshLambertMaterial( { map: skinTexture, color: 0xff0000, side: THREE.DoubleSide } ) );
				fGuy.name = "LA";
				fGuy.position.y = 0.2;
				fGuy.position.z = 0;	//0.1
				firstGuyBody.add(fGuy);

				sGuy = new THREE.Mesh( tmpLA.clone(), new THREE.MeshLambertMaterial( { map: skinTexture, color: 0x00ff00, side: THREE.DoubleSide } ) );
				sGuy.name = "LA";
				sGuy.position.y = 0.2;
				sGuy.position.z = 0;	//0.1
				secondGuyBody.add(sGuy);
				...
				loader.load( "models/Guy2/GuyRA.js", function(geometry3){
					...
					fGuy = new THREE.Mesh( tmpRA.clone(), new THREE.MeshLambertMaterial( { map: skinTexture, color: 0xff0000, side: THREE.DoubleSide } ) );
					fGuy.name = "RA";
					...
					firstGuyBody.position.y = -0.9;	//
					firstGuy.add( firstGuyBody );

					sGuy = new THREE.Mesh( tmpRA.clone(), new THREE.MeshLambertMaterial( { map: skinTexture, color: 0x00ff00, side: THREE.DoubleSide } ) );
					sGuy.name = "RA";
					sGuy.position.y = 0.2;
					sGuy.position.z = 0;	//0.1
					secondGuyBody.add(sGuy);
					secondGuyBody.position.y = -0.9;	//
					...
					loader.load( "models/Guy2/GuyH.js", function(geometry4){
						...
						eyeScreen.scale.set(0.3,0.3,0.3);
						eyeScreen.position.y = -0.3;	//1
						eyeScreen.position.z = 1.5;	//1
...
function updatePlayer(playerIndex, playerLocX, playerLocZ, playerRotY, playerQ){
	...
	if(playerIndex==1){
		...
		if(secondGuy.children[0])
			secondGuy.children[0].rotation.y = ahhRotation.y;
	}
}
